...
public class Manager extends User {
	...
	public void addClassSection() {
		...
		System.out.print("Enter Requirement Subject Name (leave blank if none): ");
		String reqName = scanner.nextLine();
		Subject reqSubject = null;
		if (!reqName.isBlank()) {
			reqSubject = findSubjectByName(reqName);
			if (reqSubject == null) {
				System.out.println("Requirement subject not found. It will be left blank.");
			}
		}
		...
		if (duplicate) {
			...
			ClassSection newClassSection = new ClassSection(classSectionId, subject, semester, maxLecturers, maxStudents, schedules, reqSubject);
	...
	/**
	 * Returns a combined list of all credit and fixed program subjects
	 */
	public List<Subject> getAvailableSubjects() {
		List<Subject> list = new ArrayList<>();
		list.addAll(Main.creditSubjects);
		list.addAll(Main.fixedSubjects);
		return list;
	}

	/**
	 * Finds a subject from either list by matching its name or code. Returns
	 * {@code null} if no match is found.
	 */
	public Subject findSubjectByName(String subjectName) {
		return Stream.concat(Main.creditSubjects.stream(), Main.fixedSubjects.stream())
				.filter(s -> s.subjectName.equalsIgnoreCase(subjectName)
						|| s.subjectCode.equalsIgnoreCase(subjectName))
				.findFirst()
				.orElse(null);
	}
	...
	class ManagerPanel extends JFrame {
		...
		private void showAddClassSectionDialog(Manager manager) {
			...
			JComboBox<String> requirementComboBox = new JComboBox<>();
			requirementComboBox.addItem("None");
			...
			classSectionDialog.add(requirementLabel);
			classSectionDialog.add(requirementComboBox);
		...
		private void showAddStudentDialog(Manager manager) {
			...
			studentDialog.add(userIdLabel);
			studentDialog.add(userIdField);
			studentDialog.add(emailLabel);
			studentDialog.add(emailField);
			studentDialog.add(passwordLabel);
			studentDialog.add(passwordField);
			studentDialog.add(nameLabel);
			studentDialog.add(nameField);
			studentDialog.add(typeLabel);
			studentDialog.add(typeComboBox);
			studentDialog.add(new JLabel("Status:"));
			studentDialog.add(statusCheckBox);
			studentDialog.add(dobLabel);
			studentDialog.add(dobField);
			studentDialog.add(studentIdLabel);
			studentDialog.add(studentIdField);
			studentDialog.add(majorLabel);
			studentDialog.add(majorField);
			studentDialog.add(new JLabel());
			studentDialog.add(submitButton);
		...
		private void showAddLecturerDialog() {
			...
			lecturerDialog.add(userIdLabel);
			lecturerDialog.add(userIdField);
			lecturerDialog.add(emailLabel);
			lecturerDialog.add(emailField);
			lecturerDialog.add(passwordLabel);
			lecturerDialog.add(passwordField);
			lecturerDialog.add(nameLabel);
			lecturerDialog.add(nameField);
			lecturerDialog.add(new JLabel("Status:"));
			lecturerDialog.add(statusCheckBox);
			lecturerDialog.add(dobLabel);
			lecturerDialog.add(dobField);
			lecturerDialog.add(lecturerIdLabel);
			lecturerDialog.add(lecturerIdField);
			lecturerDialog.add(deptLabel);
			lecturerDialog.add(deptField);
			lecturerDialog.add(new JLabel());
			lecturerDialog.add(submitButton);
		...
		public void refreshClassSectionTable() {
			try {
				...
				for (ClassSection classSection : Manager.classSections) {
					...
					String requirementName = classSection.requirement == null ? "" : classSection.requirement.subjectName;
